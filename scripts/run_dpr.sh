...
if [ "$TO_PERFORM" != "test" ]; then
    ...
    # When training a domain-general biencoder model on mMARCO-fr.
    ...
    # When training a domain-specific biencoder model on LLeQA.
...
# Testing params.
    ...
    # Testing the domain-general or domain-specific biencoder models on LLeQA.
        ...
        MODEL="maastrichtlawtech/dpr-legal-french" #in-domain
        #MODEL="antoinelouis/biencoder-camembert-base-mmarcoFR" #zero-shot
    fi
fi

# Run training or testing.
for SEED in "${SEEDS[@]}"; do
    python src/retrievers/single_dense_biencoder.py \
        --dataset $DATASET \
        --model_name "$MODEL" \
